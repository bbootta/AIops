...
| `MR-F003` | 가격차이 | `Break = \|P_front − P_bench\| > max(T_abs, \|P_bench\| × T_rel)` — 절대·상대를 **함께 정의**하고 한도는 그 중 큰 값(주1) |
...
| 무차익 (vol) | butterfly | Gatheral & Jacquier `g(k) ≥ 0` — **볼록성(c≥0)이 아니다**(주3) |
| 무차익 (vol) | calendar | 같은 k에서 만기 증가 시 total variance 비감소 |
| 커브 | par 재현 | 부트스트랩 커브로 실제 쿠폰 스케줄을 재할인해 par 금리 대조 |

> **주1** MR-F003의 한도는 `max(절대, |기준가|×상대)` 이므로 **둘 중 느슨한
> 쪽**이 한도가 된다 — 두 오차를 함께 정의해 두는 이유는, 절대치만 쓰면 대액
> 상품에서 미탐지가 나고 상대치만 쓰면 소액 상품에서 과다 BREAK가 나기
> 때문이다. 어느 한쪽만 구현하면 그 반대편이 무너진다.
>
> **주2** MR-F007의 ES 꼬리 건수는 `ceil(N(1−α))`로 고정한다. 반올림 방식을
> 바꾸면 자본이 조용히 변한다 — 변경 시 사유를 명시할 것.
>
> **주3** 변동성면 butterfly 조건을 `c ≥ 0`(볼록성)으로 구현하면 **양방향으로
> 오판한다**: 볼록해도 스큐가 가파르면 차익거래가 존재하고, 완만히 오목해도
> 무차익일 수 있다. Gatheral & Jacquier Lemma 2.2 의
> `g(k) = (1 − k·w′/2w)² − (w′²/4)(1/w + 1/4) + w″/2 ≥ 0` 을 쓴다.
> 담당 모듈: `risk_lib.market_data` (SEC-PRC-001/004) · 보고서 ops/67.
