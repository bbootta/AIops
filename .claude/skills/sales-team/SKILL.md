...
- **G1 무발송 원칙**: 에이전트는 발송 도구를 실행하지 않는다. 발송
  ...
  상신한다.
- **3중 게이트 fail-closed**: QA(G3) → 컴플라이언스 단계(G2, 해당 시
  한국 분기 G7과 EU LIA G10 포함) → 딜리버러빌리티 프리플라이트(G5) PASS 기록
...
- **관할 규율**: "EU" 단일 세그먼트 금지, 국가 단위로 판정한다. KB09
  회피 등급 국가(독일, 오스트리아, 이탈리아, 스페인, 폴란드, 스위스,
  중국)와 팀 정책 회피(캐나다: KB09 등급은 조건부), 한국은 콜드 메일
  대신 대체 채널 플레이로 접근한다. 경계 사안은 legal-team 하네스로 에스컬레이션하고 회신 전까지
  해당 레코드만 분리 보류한다.
