export const meta = {
  ...
  whenToUse: '콜드 아웃리치 캠페인 기획과 발송 준비 요청 시. args: { region: "US | EU-<국가코드> | UK | JP | SG | AU | CA | CH | CN | KR", segment: "타깃 세그먼트 설명(예: 미국 자산운용사의 리서치 조직)", tier: 1|2|3, goal: "목표(예: 4주 내 미팅 5건)", list_source: "리스트 출처(선택)", date: "YYYY-MM-DD(오늘)" }',
  phases: [
...
if (!REGION) throw new Error('args.region이 필요합니다: US | EU-<국가코드> | UK | JP | SG | AU | CA | CH | CN | KR')
if (!/^(US|UK|JP|SG|AU|CA|CH|CN|KR|EU-[A-Z]{2})$/.test(REGION)) throw new Error('region 형식 오류: US | EU-<국가코드 2자리 대문자> | UK | JP | SG | AU | CA | CH | CN | KR 중 하나여야 합니다 (예: EU-FR). EU 단일 세그먼트는 금지, 반드시 국가 단위로 지정')
...
// 콜드 메일 금지, 대체 채널 트랙 강제 대상:
// KB09 §8.1 회피 등급(독일, 오스트리아, 이탈리아, 스페인, 폴란드, 스위스, 중국)
// + 팀 정책 회피(캐나다: KB09상 조건부이나 CASL 리스크로 팀 기본값 회피)
// + 한국(G7: 기본값 대체 채널, KB06 A/B 유형 1:1 사업 제안 소량 수동 발송만 alt-channel-play 안에서 예외 취급)
const AVOID_REGIONS = ['EU-DE', 'EU-AT', 'EU-IT', 'EU-ES', 'EU-PL', 'CH', 'CN', 'CA', 'KR']
...
const BRIEF_SCHEMA = {
  ...
  properties: {
    campaign_slug: { type: 'string', pattern: '^[a-z0-9-]+$', description: '캠페인 영문 슬러그(소문자, 숫자, 하이픈만. 예: us-asset-mgmt-research)' },
...
const COMPLIANCE_SCHEMA = {
  ...
  properties: {
    ...
    lia_file: { type: 'string', description: 'EU/영국 레코드 포함 시 작성한 LIA 초안 경로(국가별 복수면 세미콜론으로 구분해 전부 나열, 해당 없으면 빈 문자열)' },
...
const brief = await agent([
  ...
  '2) 트랙 분기: region이 회피 등급 국가(독일, 오스트리아, 이탈리아, 스페인, 폴란드, 스위스, 중국, 캐나다)이거나 한국이면 콜드 메일 트랙 대신 대체 채널 트랙(alt-channel)을 선언하라. 한국은 G7에 따라 기본값이 대체 채널이며, KB06 A/B 유형(1:1 사업 제안) 소량 수동 발송만 alt-channel-play 안에서 예외 취급한다.',
...
const copyPromptBase = [
  ...
  ALT_TRACK
    ? [
      '작성 범위(대체 채널 트랙: 콜드 메일 대량 발송이 없으므로 이메일 시퀀스 카피는 작성하지 않는다):',
      '- LinkedIn 커넥션 노트(영문)와 InMail/DM 시퀀스 카피, 이벤트/전시회/파트너 접점용 메시지.',
      '- 콜 오프닝과 20초 보이스메일 스크립트(해당 슬롯이 있으면).',
      REGION === 'KR' ? '- 한국 A/B 유형(1:1 사업 제안) 소량 수동 발송 후보가 플레이에 있으면: 국문 서신을 수신자 1명당 건별 개별 작성하라(템플릿 대량 변형 금지, KB06 §4.2 체크리스트 준수). 리서치 산출물 기반으로 수신자별로 따로 써라.' : '',
      '- A/B 테스트 변형은 단일 변수로만.',
    ].join('\n')
    : [
      '작성 범위(터치맵의 모든 슬롯):',
  ...
  '',
  '카피를 ' + (ALT_TRACK ? 'templates/sales/alt-channel-play.md의 카피 섹션 구조로 ' : 'templates/sales/multichannel-sequence.md의 카피 섹션 구조로 ') + CAMP_DIR + '/copy.md 에 저장하라.',
].join('\n')
...
if (qa.verdict === 'FAIL') {
  ...
  if (!redesign) log('재설계 보고 생성 실패: sales-lead 에이전트를 수동으로 재실행할 것')
  return {
    status: 'failed',
    campaign_id: CAMPAIGN_ID,
    reason: 'QA 게이트 최종 FAIL(반려 2회 소진). 발송 패키지는 조립되지 않았다(fail-closed).',
    qa_failures: qa.failures,
    artifacts: { brief: brief.brief_file, list: list.list_file, touchmap: touchmap.touchmap_file, copy: copy.copy_file, qa_record: qa.record_file, redesign_report: redesign ? CAMP_DIR + '/redesign-report.md' : null },
    redesign: redesign || '(재설계 보고 생성 실패: sales-lead 재실행 필요)',
    po_todo: [
      redesign ? '재설계 보고(' + CAMP_DIR + '/redesign-report.md) 검토 후 재설계 방향 승인' : '재설계 보고 생성 실패: sales-lead 에이전트로 재설계 보고를 다시 생성',
      '승인 후 cold-outreach-campaign 재실행',
    ],
  }
}
...
const compliance = await agent([
  ...
  '- EU/영국 레코드가 있으면 templates/sales/lia-record.md 구조로 LIA 초안(3단 테스트: 목적 정당성/필요성/이익형량)을 국가 단위로 작성하고 PO 승인 상신 항목으로 표시하라. 파일명은 lia-record.md의 ID 규약을 따른다: ' + COMP_DIR + '/LIA-' + DATE8 + '-<국가코드>-<순번>.md (예: LIA-' + DATE8 + '-FR-01.md). 생성한 LIA ID를 해당 레코드들의 LIA ID 필드에 기재하게 하고(G9), 전체 경로를 lia_file에 세미콜론으로 구분해 나열하라. LIA 없음을 이유로 게이트를 형식 통과시키는 것은 금지다(G10).',
  '',
  '판정 규칙:',
...
const RELEASE_CONDITIONS = [
  '터치 2~N 릴리스 조건(발송 활성 기간 매 영업일, 후속 터치 발송 예정일 포함. 워크플로가 아니라 sales-ops-review daily 모드로 판정, G6):',
  ...
  '4) templates/sales/touch-release-checklist.md 기반 PASS 기록(' + COMP_DIR + '/' + CAMPAIGN_ID + '-touch<터치번호>-release.md)이 남지 않은 터치는 발송 불가(fail-closed)',
...
if (!pkg) throw new Error('발송 패키지 조립 실패: ' + OUTBOX_DIR + '/send-package.md 미생성(fail-closed)')
...
if (!record) throw new Error('캠페인 레코드 생성 실패: ' + CAMP_DIR + '/campaign-record.md 미생성')
...
return {
  ...
  touch_release_guidance: '후속 터치(터치 2~N) 릴리스는 이 워크플로에서 반복 실행할 수 없다. 발송 활성 기간 매 영업일(후속 터치 발송 예정일 포함) sales-ops-review를 daily 모드로 실행해 터치 릴리스 게이트(suppression 델타 재대조, 발송 도구 동기화, 잔여 일일 한도, 직전 터치 지표)를 판정하라(G6). touch-release-checklist PASS 기록 없이 해당 터치는 발송 불가이며, 캠페인 개시와 볼륨 증량 후 3일간 daily 모드는 필수다(KB02 §6.2).',
  po_todo: [
