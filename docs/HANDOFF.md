...
**최종 커밋** 종합보고서 화면 + 전 원장 자동 시각화 · 작업 트리 clean
...
| `810f5b5` | 보고서 그룹 신설(통제센터 위) + Executive Report 화면 |
| (직전) | 종합보고서로 개명 · KRI 카드 · 차트 원시함수 6종 · 원장 자동 시각화 |
...
**에이전틱 UI 메뉴 최상단은 `보고서` 그룹이다.** 그 아래 `종합보고서`가
...
**시각화** — 실무진 보고서(ops/*.html)가 그리는 것을 화면도 그린다. 차트
원시함수는 `bars`·`stackBars`·`donut`·`heat`·`waterfall`·`gauge`·`kriCards`·
`hbars`·`multiLine`·`sparkline`이고 **전부 인라인 SVG**다 — 외부 차트 라이브러리를
쓰면 아티팩트 CSP에서 차단돼 배포본에서만 조용히 사라진다.

원장 화면은 `autoChart`가 스펙에서 축을 골라 자동으로 그린다(107장 중 72장).
화면마다 손으로 붙이지 않으므로 새 원장이 늘어도 따라온다. 두 가지를 지킨다:

- **미리보기(12행)로는 그리지 않는다.** 2,980행 원장의 12행 막대는 표본이지
  분포가 아니다. 전량이 실린 프레임(`D.data`, 107장 중 92장)만 쓰고, 잘려 있으면
  그리지 않는다 — 그럴듯하게 그리는 쪽이 안 그리는 것보다 나쁘다.
- **수치축은 우선순위(`PREF`)로 고른다.** 컬럼 순서로 첫 매치를 잡으면
  `n_exposures`가 `rwa`보다 앞이라는 이유로 RWA 원장이 "익스포저 수"를 그린다 —
  실제로 그렇게 그렸다. 건수는 금액이 하나도 없을 때만 쓴다.
