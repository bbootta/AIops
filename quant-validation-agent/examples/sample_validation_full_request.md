...
|---:|---|
| 0 | 정상 종료 |
| 1 | `policy-governance --exit-on-yellow` 사용 시 lock 부재/불일치 (소프트 경고) |
| 2 | `check`에서 위험 키워드 탐지 |
| 3 | `check`에서 PII 탐지 |
| 4 | 입력/스키마 오류 (`validate*`, `report` 입력 누락 등) |
| 5 | `note add`에서 PII/위험 키워드로 차단 |
| 6 | 정책 schema 검증 실패, `policy-governance` 매니페스트 거버넌스 위반, 또는 `summary --fail-on-red` 사용 시 어느 입력이 Red |
| 7 | `policy-governance --require-lock` 사용 시 lock 미존재/불일치 (하드 실패) |
