...
SHARED_REFERENCE_TABLES: dict[str, str] = {
    ...
    "macro_indicator": (
        "거시·금융지표 관측치다. 같은 시점 같은 지표는 기관과 무관하게 한 값이다. "
        "값 자체는 지표 원장 모수에서 파생한 계열이며 외부 피드가 아니다(basis 전건 '파생'). "
        "이 판정은 계열을 **기관 오프셋을 더하기 전 시드**로 만들 때만 성립한다. "
        "기관 시드로 만들면 기관코드 없는 같은 기본키에 기관마다 다른 값이 생기고, "
        "두 기관 원장을 합칠 때 한쪽이 조용히 덮인다. "
        "check_shared_reference_agreement 가 그 상태를 잡는다."),
    # 신용 규제 계수·요건
