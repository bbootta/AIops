...
def _br01(ctx) -> BuiltForm:
    ...
    L = [
        ...
        FormLine("2100", "신용리스크", 1, "KRW",
                 rwa["credit_internal"] + float(rwa.get("structured_total", 0.0)),
        ...
        # 집합투자증권·유동화는 **신용리스크 안**이다 (CRE60 · CRE40). 시장·
        # 운영과 나란한 별도 리스크 범주로 올리면 편제를 잘못 주장하게 된다.
        FormLine("2140", "집합투자증권", 2, "KRW",
                 float(rwa.get("fund", 0.0)),
                 formula="기초자산투과법(LTA) · 위임기준법(MBA) · 1250% 대체",
                 citation="CRE60", source_module="risk_lib.datamodel.funds"),
        FormLine("2150", "유동화 익스포저", 2, "KRW",
                 float(rwa.get("securitisation", 0.0)),
                 formula="SEC-IRBA → SEC-ERBA → SEC-SA 계층, 위험가중치 하한 적용",
                 citation="CRE40 · CRE41 · CRE44",
    ...
    checks = [
        ...
        _sum_check("신용RWA = SA+IRB+CCR+집합투자증권+유동화", L, "2100",
...
def _br11(ctx) -> BuiltForm:
    ...
    checks = [
        # 아래 두 대사도 양변이 같은 `reserve_requirement()` 반환값에서 나오므로
        # **자료로는 틀릴 수 없다** — 이전 검증(`min(0, 건별합산 − 합계기준)`)과
        # 같은 항등식 계열이다 (지적 F-602 · F-702). 다만 잡는 대상이 다르다:
        # 라인 3000을 건별 합산으로 되돌리는 **배선 회귀**는 잡는다. 실제로 그
        # 회귀 시험을 tests/test_regulatory.py에 넣었다. 자료 오류를 잡는 통제로
        # 읽지 말 것 — 그 역할은 서식 간 대사(cross_form)가 한다.
