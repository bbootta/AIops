...
def _b2403(ctx) -> tuple[list[FormLine], list[FormCheck]]:
    ...
    L = [
        ...
        FormLine("1030", "대손준비금 순차액 합계", 0, "KRW",
                 reserve_net_gap(book), formula="최저적립액 − 충당금 (부호 있는 순차액 — 음수는 초과적립. 부문 합이 전행과 일치하도록 max(0,·)를 걸지 않는다)",
                 citation="은행업감독규정 제29조 제2항", source_module=_M_RDM,
                 is_subtotal=True),
    ...
    for i, prod in enumerate(PRODUCTS, start=1):
        ...
        L += [
            ...
            FormLine(str(base + 30), "대손준비금 순차액", 2, "KRW",
                     reserve_net_gap(s), formula="최저적립액 − 충당금 (부호 있는 순차액 — 음수는 초과적립. 부문 합이 전행과 일치하도록 max(0,·)를 걸지 않는다)",
                     citation="은행업감독규정 제29조 제2항", source_module=_M_RDM),
