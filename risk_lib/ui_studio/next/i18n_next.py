...
_t("ng_gate",
   ...
   '요구 {v}||Required {v}',
   '여유 {v}||Headroom {v}',
   '제약 {tier} {v}||Binding {tier} {v}',
   '완충자본 미달 {n}종, 배당·성과급 제한||{n} buffer shortfalls, distributions restricted',
   '전 계층 요구 충족||All tiers meet the requirement',
   '심각 시나리오 {q}||Severely adverse {q}',
   '고정이하여신비율 {v}||Substandard or below {v}',
   'NSFR {v}||NSFR {v}',
   '최저 {v}||Minimum {v}',
   '총 {n}건||{n} in total',
   '서식 {n}장||{n} forms',
   ...
   '기본자본||Tier 1',
   '총자본||Total capital',
   '승인 {approved} · 반려 {returned} · 보류 사유 {kinds}종 · 제출 {reviewed}/{total}||approved {approved} · returned {returned} · {kinds} hold reasons · submitted {reviewed}/{total}',
