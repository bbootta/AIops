...
TRACE: dict[str, tuple[str, tuple[tuple[str, str], ...], str]] = {
    # ---- AIG · AI 거버넌스 -------------------------------------------------
    "AIG-001": ("부분", (("table", "agent_registry"), ("screen", "G 에이전트")),
                "에이전트 등록 원장은 있으나 위험등급 분류 체계는 없다"),
    ...
    "RDM-007": ("부분", (("screen", "집계·대사 예외 큐"),),
                "예외 큐 표시까지 — 조치 워크플로는 없다"),
    ...
    "DAT-004": ("부분", (("table", "rdm_source_contract"), ("module", "risk_lib.archive")), ""),
    ...
    "PLT-016": ("부분", (("screen", "Kill Switch"), ("table", "agent_killswitch")),
                "전역 정지만 — 범위형(scoped) 정지는 원장 설계만 있다"),
    "PLT-017": ("반영", (("table", "gov_approval"),
                        ("test", "test_approvals_enforce_segregation_of_duties")), ""),
    "PLT-018": ("부분", (("screen", "증빙 계보 · 7단계"), ("module", "risk_lib.archive")),
                "지문·판 대조는 있으나 화면 드릴다운·버전 diff 는 없다"),
    # ---- BNK ---------------------------------------------------------------
    ...
    "SEC-MKT-003": ("부분", (("module", "risk_lib.attribution"),), "PLA는 있으나 한도 연결은 없다"),
