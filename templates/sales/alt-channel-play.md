...
## 7. 한국 A/B 유형 1:1 사업 제안 서신 (해당 시에만)

> G7이 허용하는 유일한 한국 이메일 경로다: 무동의 발송 중 A(순수 거래·제휴 문의) /
> B(1:1 맞춤 사업 제안) 유형의 국문 서신을 **소량 수동 발송**한다. 템플릿 대량
> 발송(C/D 유형)은 무조건 반려 [G7]. 수신자 1명당 건별 개별 작성이 강제된다
> (KB06 §4.2 체크리스트 준수).

| 수신자 | 회사/직함 | 유형(A/B) | KB06 §4.2 체크 | 서신 파일 | 발송(PO, 수동) |
|---|---|---|---|---|---|
| {이름} | {회사/직함} | {A/B} | {통과/미통과} | {경로} | {예정일} |

- 건별 서신 본문: {수신자별 개별 작성 국문 서신. 리서치 산출물의 시그널과 가설 기반}
- 발송 전 확인: 경계 사안(광고성 판단이 갈리는 문안)은 legal-team 회신 전 발송 금지
