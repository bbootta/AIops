...
# 위험프리미엄 0 하한(사용자 지시)을 넣었으나 이 파이프라인의 집계는 움직이지
# 않는다. 하한은 **빈 행만** 채우기 때문이다. 파이프라인은 '전체' 회수유형을
# `_provisional_discount_rates` 의 참고치(타행 실측)로 이미 채우므로 하한이
# 닿지 않고, 비어 있던 '무위험회수' 만 R_f 로 채워진다. 두 값이 달라 서열 검사도
# 판정을 유지한다.
#
# 하한이 실제로 무는 곳은 참고치도 승인 R_M 도 없는 경로다. 그때는
