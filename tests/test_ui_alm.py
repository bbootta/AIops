...
@browser_only
def test_irrbb_screen_discloses_the_proxy_and_the_blank_shock(alm_text):
    """충격폭이 원화 계정 값이 아니라는 사실이 화면에 있다."""
    txt = alm_text["금리리스크"]
    assert "프록시" in txt
    assert "KRW 산출에 대용" in txt
    assert "공란" in txt and "지어 채우지 않는다" in txt
    # 판정 기준값이 원장에 없으므로 아웃라이어는 판정하지 않는다
    assert "미판정" in txt
