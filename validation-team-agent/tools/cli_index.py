...
CLI_MODULES: list[tuple[str, str]] = [
    ...
    ("tools.domain_criteria", "도메인 업무요건 → 적합성검증 기준 항목 원장 (근거 실재성 강제)"),
    ...
    ("tools.climate_criteria", "기후리스크 업무요건 72건 → 적합성검증 기준 항목 원장 (원문 파싱·근거 실재성 강제)"),
    ("tools.gen_climate_criteria", "기후리스크 기준 항목 원장 생성기: 요건정의서 원문이 바뀌면 재실행"),
    ("tools.ai_risk_criteria", "AI 리스크 업무요건 76건 → 적합성검증 기준 항목 원장 (원문 파싱·근거 실재성 강제)"),
    ("tools.gen_ai_risk_criteria", "AI 리스크 기준 항목 원장 생성기: 문서 세트 원문이 바뀌면 재실행"),
    ("tools.climate_recalc", "기후리스크 합성 사례 독립 재계산: 손실·재무·PD·EAD·ECL·RWA·자본 단계별 대사 (CLR-18-02)"),
    ...
    ("tools.reg_rules", "규제 규칙 카탈로그 — 근거수준·원문주기·유효일자"),
